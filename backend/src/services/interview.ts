...
export async function handleInterview(input: {
  ...
  if (!input.message) {
    ...
    await appendMessage(conversation.id, { role: 'assistant', content: first.prompt })
    return {
  ...
  await appendMessage(conversation.id, { role: 'user', content: input.message })
  ...
  if (asked >= bank.length) {
    ...
    await appendMessage(conversation.id, { role: 'assistant', content: wrap })
    return {
      conversationId: conversation.id,
      track: input.track,
      question: wrap,
  ...
  try {
    const retrieval = await retrieveCandidateContext(current.prompt)
  ...
  await appendMessage(conversation.id, { role: 'assistant', content: cleaned })
  return {
    conversationId: conversation.id,
    track: input.track,
    question: cleaned,
